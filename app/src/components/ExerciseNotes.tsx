...
export default function ExerciseNotes({ exerciseId, notes }: { exerciseId: number; notes?: string }) {
    ...
    return (
      <div className="mt-2 flex flex-col gap-1.5">
        <textarea
          ...
          placeholder="Form cues, seat height, machine settings…"
          rows={2}
        ...
        <div className="flex gap-2">
          <Button className="flex-1" onClick={save}>Save note</Button>
          <Button variant="secondary" className="flex-1" onClick={() => { setDraft(notes ?? ''); setEditing(false); }}>Cancel</Button>
        </div>
      </div>
    );
  }
  ...
  return notes ? (
    <button onClick={() => setEditing(true)} className="mt-1.5 block text-left text-xs text-slate-400 italic">
      📝 {notes}
    </button>
  ) : (
