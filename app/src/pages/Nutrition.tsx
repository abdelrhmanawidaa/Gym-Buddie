...
import { Card, PageHeader, Button, Input, ProgressBar, EmptyState } from '../components/ui';
...
export default function Nutrition() {
  ...
  async function addPreset(p: (typeof QUICK_PRESETS)[number]) {
    await db.foodLogs.add({ date, name: p.name, calories: p.calories, protein: p.protein, carbs: p.carbs, fat: p.fat, createdAt: Date.now() });
  }
  ...
  async function saveGoal(next: { calories: number; protein: number; carbs: number; fat: number }) {
  ...
  return (
    <div className="pb-4">
      <PageHeader title="Nutrition" subtitle="Calories & protein" />
      ...
      <div className="flex flex-col gap-3 px-4">
        ...
        <Card>
          ...
          {showGoal ? (
            <GoalForm goal={goal} onSave={saveGoal} onCancel={() => setShowGoal(false)} />
          ) : (
        ...
        <div>
          <p className="mb-2 text-sm font-medium text-slate-300">Quick add</p>
        ...
        <div>
          ...
          {(!logs || logs.length === 0) && <EmptyState text="Nothing logged for this day yet." />}
...
function GoalForm({
  ...
  onSave: (v: { calories: number; protein: number; carbs: number; fat: number }) => void;
  onCancel: () => void;
}) {
